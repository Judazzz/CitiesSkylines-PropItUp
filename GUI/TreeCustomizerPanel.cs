...
namespace PropItUp.GUI
{
    public class TreeCustomizerPanel : UIPanel
    {
        ...
        public void PopulateIncludedTreesFastList()
        {
            ...
            if (PropItUpTool.config.enable_debug)
            {
                DebugUtils.Log($"TreeCustomizerPanel: OriginalFastList populated with {selectedBuildingTreeList.Count} trees for building '{BuildingSelectionTool.instance.m_selectedBuilding.name}'.");
            }
        }
